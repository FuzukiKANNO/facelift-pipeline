...
### 3D福笑い / HMD重畳の準備（進行中）
最終目標: 現実の鼻・口などの立体物に、HMD で各パーツ3Dモデルを重ねて表示。
まず各パーツを Unity で個別に扱えるようにする。

- 表現は **Gaussian Splat のまま**採用（メッシュ化は暗く小さいパーツで破綻し写実性が落ちたため）。
- HMD は **PC 接続**（このPCの GPU で描画）想定 → UnityGaussianSplatting が実用的。
- パーツ構成（福笑い）: `eyebrow_eye_right` / `eyebrow_eye_left` / `nose` / `mouth`
  （`segment_gaussians.py --preset fukuwarai`）。
- `scripts/recenter_gs_parts.py` で各パーツを重心原点に再センタリング＋floater除去し、
  標準 3DGS 形式で `unity_parts/` に出力（元位置は manifest.json）。
- Unity 取込手順: [docs/unity_gaussian_splatting.md](docs/unity_gaussian_splatting.md)

追加スクリプト:
| ファイル | 用途 |
|----------|------|
| `scripts/recenter_gs_parts.py` | パーツを重心原点へ再センタリング＋floater除去（Splat用・推奨） |
| `scripts/gs_parts_to_mesh.py` | パーツをメッシュ(glb/obj)化（メッシュが要る場合のみ） |

**次にやること**: `unity_parts\*.ply` を UnityGaussianSplatting で取り込み（docs 参照）、
掴み用コライダーを付け、現実オブジェクトへの重畳・実寸スケール合わせに進む。
